...
const Search: React.FC<Props> = ({ words, setWords }) => {
  ...
  const handleSave = (data: Data) => {
    if (!userInfo) {
      ...
      // let i = 0;
      // if ()
      // while (i < data.meanings.length) {
      //   filteredMeanings
      // }
      // for (let i = 0; i < data.meanings.length; i++) {
      //   if (i === 0) {
      //     filteredMeanings.push(data.meanings[i]);
      //   } else {
      //     const length =
      //       filteredMeanings.length > data.meanings.length
      //         ? data.meanings.length
      //         : filteredMeanings.length;
      //     for (let j = 0; j < length; j++) {
      //       console.log(`filtered: ${filteredMeanings[j].partOfSpeech}
      //       data: ${data.meanings[i].partOfSpeech}
      //       res: ${
      //         filteredMeanings[j].partOfSpeech !== data.meanings[i].partOfSpeech
      //       }`);

      //       if (
      //         !(
      //           data.meanings[i].partOfSpeech ===
      //           filteredMeanings[j].partOfSpeech
      //         )
      //       ) {
      //         console.log('here!');
      //         filteredMeanings.push(data.meanings[i]);
      ...
      const word: WordProp = {
        ...
        pronunciation: src,
        meanings: data.meanings.slice(0, 3),
      };
