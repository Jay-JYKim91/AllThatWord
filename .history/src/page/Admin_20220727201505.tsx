...
const Admin: React.FC<Props> = ({ words, setWords, folders, setFolders }) => {
  ...
  let filteredWords = currentFolder.words;
  console.log(folders[selectedFolder]);
  let selectedFolderObj = folders[selectedFolder] as Folder;
  console.log(selectedFolderObj);
  let editedFolderName = selectedFolderObj.name;
  ...
  const handleSelectFolder = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    filteredWords = thisFolder.words;
    displayForm('hidden');
  };

  useEffect(() => {
    selectedFolderObj = folders[selectedFolder] as Folder;
    editedFolderName = selectedFolderObj.name;
  }, [selectedFolder]);
  ...
  const handleDeleteFolder = (folderId: string) => {
    setSelectedFolder(
      folders
        ? Object.keys(folders).filter((folder) => folder.id !== folderId)[0]
        : ''
    );
    setFolders((folders) => {
      const updated: LooseObj = { ...folders };
      ...
      <div className="my-2 flex justify-between items-center">
        ...
        >
          <input
            ...
            onChange={(event) => (editedFolderName = event.target.value)}
          />
