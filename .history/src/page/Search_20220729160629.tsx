...
const Search: React.FC<Props> = ({ words, setWords, folders, setFolders }) => {
  ...
  const handleDelete = (id: string) => {
    const thisWord = words[id] as { folderId: string };
    repo.removeWord(userInfo!.uid, id, thisWord);
  };
  ...
  const definitionArr: string[] = [];
  const synonymsArr: string[] = [];
  const antonymsArr: string[] = [];
    ...
    <div className="px-6 md:px-9 lg:px-12 py-4 md:py-6 lg:py-8">
      ...
      <div className="py-4 max-w-2xl m-auto dark:text-white">
        ...
        {data.meanings.map((meaning: Meaning, index: number) => {
          if (index > 0) {
            definitionArr.push(data.meanings[index - 1].partOfSpeech);
          }

          return (
            !definitionArr.includes(meaning.partOfSpeech) && (
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap text-neutral-600 dark:text-primary-900">
                        {meaning.synonyms.map((synonym, index) => {
                          if (index > 0) {
                            synonymsArr.push(meaning.synonyms[index - 1]);
                          }

                          const url = `/search/${synonym}`;
                          return (
                            !synonymsArr.includes(synonym) && (
                              <span
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap text-neutral-600 dark:text-primary-900">
                        {meaning.antonyms.map((antonym, index) => {
                          if (index > 0) {
                            antonymsArr.push(meaning.antonyms[index - 1]);
                          }

                          const url = `/search/${antonym}`;
                          return (
                            !antonymsArr.includes(antonym) && (
                              <span
                                className="mr-1 mb-1 px-2 bg-neutral-200"
                                key={antonym}
                              >
