...
import { Word, Meaning } from './Search';
...
const Admin: React.FC<Props> = ({ words, setWords }) => {
  ...
  const handleMeaningsForFlashcard = (meanings: Meaning[]) => {
    const processed: string[] = [];
    
    Object.keys(words).map((key) => {
      const word = words[key] as Word;
      for (let i = 0; i < word.meanings.length; i++) {
        processed.push(word.meanings[i].definitions[0].definition);
      }
      return;
    });
    return processed
  ...
  return (
    <div className="px-6 md:px-9 lg:px-12 py-4 md:py-6 lg:py-8">
      ...
      <div>
        ...
        {viewMode === 'flashcard' && (
          <div className="h-[50vh] overflow-hidden relative my-10">
            <button
              type="button"
            ...
            {Object.keys(words).map((key, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="card__face card__face--back flex flex-col justify-center align-middle">
                    { handleMeaningsForFlashcard(word.meanings)
                    word.meanings.map((meaning) => {
                      const result = handleMeaningsForFlashcard(meaning);
                      console.log(result);
                      return (
                        <div key={meaning.partOfSpeech}>
                          <p className="text-primary-900 font-bold text-left">
                            {meaning.partOfSpeech.toUpperCase()}
                          </p>
