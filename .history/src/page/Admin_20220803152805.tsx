...
const Admin: React.FC<Props> = ({ words, setWords, folders, setFolders }) => {
  ...
  const [filteredWords, setFilteredWords] = useState(
    Object.keys(defaultFolder).length !== 0
      ? (defaultFolder as Folder).words
      : {}
  );
  let selectedFolderObj =
    selectedFolderId !== '' ? (folders[selectedFolderId] as Folder) : {};
  const [editedFolderName, setEditedFolderName] = useState(
    Object.keys(selectedFolderObj).length !== 0
      ? (selectedFolderObj as Folder).name
      : ''
  );
  ...
  useEffect(() => {
    if (isDeletedFolder) {
      ...
      setIsFoldersEmpty(Object.keys(folders).length === 0);
    }
  ...
  const submitEditFolder = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const folder: Folder = {
      id: (selectedFolderObj as Folder).id,
      name: editedFolderName,
      words: (selectedFolderObj as Folder).words || {},
    };
  ...
  const handleDeleteFolder = (folderId: string) => {
    ...
    if (isConfirmed) {
      ...
      const tem = Object.keys(tempFolders)
        ...
        .reduce((obj, key) => {
          const object: LooseObj = obj;
          object[key] = tempFolders[key];
          return object;
        }, {});
      setSelectedFolderId(
  ...
  return (
    <div className="px-6 md:px-9 lg:px-12 py-4 md:py-6 lg:py-8 max-w-2xl m-auto">
      ...
      {!isFoldersEmpty ? (
        <div>
          <div className="text-right">
            <select
              ...
              className="border border-primary-900 rounded my-4 w-[160px] p-2 focus:outline-none"
            >
          ...
          <div className="my-2 flex justify-between items-center">
            <p
              className="font-heading_font text-2xl dark:text-white"
              id="folderName"
            >
              {(selectedFolderObj as Folder).name}
            </p>
            ...
            <div id="editAndDeleteFolderBtn">
              <button type="button" onClick={handleEditFolder}>
                <AiFillEdit className="text-2xl text-primary-900 mr-2 dark:text-white" />
              </button>
              <button
                type="button"
                onClick={() =>
                  handleDeleteFolder(`${(selectedFolderObj as Folder).id}`)
                }
              >
                <RiDeleteBin5Fill className="text-2xl text-primary-900 dark:text-white" />
              </button>
            </div>
            ...
            >
              ...
              <button type="submit" id="submitBtn">
                <BsCheckLg className="text-2xl text-primary-900 dark:text-white" />
              </button>
            </form>
          </div>
            ...
            {(!filteredWords || Object.keys(filteredWords).length === 0) && (
              <div className="my-8 text-center text-xl">
                <p className="dark:text-neutral-200">
                  There is no word in this folder.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="my-12 text-center text-xl">
          <p className="dark:text-neutral-200">No word saved</p>
        </div>
      )}
    </div>
  );
