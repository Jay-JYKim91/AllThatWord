...
const Admin: React.FC<Props> = ({ words, setWords, folders, setFolders }) => {
  ...
  console.log(`defaultFolder: ${folders ? Object.keys(folders)[0] : ''}`);
  const [selectedFolder, setSelectedFolder] = useState(defaultFolder);
  console.log(`selectedFolder: ${selectedFolder}`);
  const currentFolder = folders[defaultFolder] as Folder;
  const [filteredWords, setFilteredWords] = useState(
    currentFolder ? currentFolder.words : ''
  );
  console.log(folders[selectedFolder]);
  let selectedFolderObj = folders[selectedFolder]
    ? (folders[selectedFolder] as Folder)
    : '';
