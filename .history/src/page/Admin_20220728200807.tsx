...
const Admin: React.FC<Props> = ({ words, setWords, folders, setFolders }) => {
  ...
  useEffect(() => {
    selectedFolderObj =
      selectedFolderId !== '' ? (folders[selectedFolderId] as Folder) : {};
    setEditedFolderName(selectedFolderObj ? selectedFolderObj.name : '');
  }, [selectedFolderId]);
      ...
      {!isFoldersEmpty ? (
        <div>
          <select
            ...
            className="border border-primary-900 rounded my-4 mr-0 w-[50%] p-2"
          >
        ...
        <p className="my-4 text-center text-xl">No word saved</p>
      )}
    </div>
  );
};
