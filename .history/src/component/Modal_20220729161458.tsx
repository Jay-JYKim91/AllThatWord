...
const Modal: React.FC<Props> = ({
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (arr.includes(folderName)) {
      window.alert(`'${folderName}' folder is already existed.`);
    } else {
      const folder: Folder = {
        id: date,
        name: folderName,
  ...
  return (
    <div
      className="bg-white border rounded border-primary-900 hidden absolute right-0 pt-4 min-w-[230px] z-10 dark:border-white"
      id="modal"
    >
      ...
      <form onSubmit={handleSubmit} className="hidden px-4 my-2" id="form">
        <input
          ...
          className="border rounded-tl rounded-bl px-1 border-primary-900 w-full focus:outline-none dark:text-primary-900"
          id="folderNameInput"
        />
        <button
          type="submit"
          className="py-1 px-2 bg-primary-900 text-white rounded-tr rounded-br font-medium"
        >
          ADD
        </button>
      </form>
        ...
        {Object.keys(folders).map((key) => {
          ...
          return (
            <li key={folder.id} className="my-1">
              ...
              />
              <label htmlFor="selectedFolder" className="dark:text-black">
                {folder.name}
              </label>
            </li>
          );
      ...
      <button
        type="button"
        className="w-full bg-primary-900 text-white py-2 mt-2 rounded-br rounded-bl"
        onClick={handleSave}
      >
        SAVE
      </button>
